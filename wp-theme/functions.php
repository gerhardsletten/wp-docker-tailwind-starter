...
function smartwp_remove_wp_block_library_css()
{
  ...
  if (
    !is_admin() &&
    (is_front_page() || is_home() || in_array($url, $urlList))
  ) {
...
add_action("customize_register", function ($wp_customize) {
  ...
  $wp_customize->add_setting("themeconfig_facebook", [
    "default" => "",
  ]);

  $wp_customize->add_control("themeconfig_facebook", [
    "label" => "Facebook url",
    "section" => "theme_settings",
    "type" => "text",
  ]);
  $wp_customize->add_setting("themeconfig_instagram", [
    "default" => "",
  ]);

  $wp_customize->add_control("themeconfig_instagram", [
    "label" => "Instagram url",
    "section" => "theme_settings",
    "type" => "text",
  ]);

  $wp_customize->add_setting("themeconfig_footer_text", [
    "default" => "",
  ]);

  $wp_customize->add_control("themeconfig_footer_text", [
    "label" => "Footer text",
    "section" => "theme_settings",
    "type" => "textarea",
  ]);
...
function register_html5_menu()
{
  register_nav_menus([
    ...
    "header-menu" => __("Main menu", "html5blank"),
  ]);
}
...
\*------------------------------------*/ // Add Actions // add_action('init', 'html5blank_header_scripts'); // Add Custom Scripts to wp_head // add_action('wp_print_scripts', 'html5blank_conditional_scripts'); // Add Conditional Page Scripts // add_action('get_header', 'enable_threaded_comments'); // Enable Threaded Comments
add_action("wp_enqueue_scripts", "html5blank_styles"); // Add Theme Stylesheet
add_action("init", "register_html5_menu"); // Add HTML5 Blank Menu
add_action("init", "create_post_type_html5"); // Add our HTML5 Blank Custom Post Type
add_action("widgets_init", "my_remove_recent_comments_style"); // Remove inline Recent Comment Styles from wp_head()
add_action("init", "html5wp_pagination"); // Add our HTML5 Pagination // Remove Actions
...
add_filter("image_send_to_editor", "remove_thumbnail_dimensions", 10); // Remove width and height dynamic attributes to post images // Remove Filters
remove_filter("the_excerpt", "wpautop"); // Remove <p> tags from Excerpt altogether // Shortcodes
add_shortcode("html5_shortcode_demo", "html5_shortcode_demo"); // You can place [html5_shortcode_demo] in Pages, Posts now.
add_shortcode(
  "html5_shortcode_demo_2",
  "html5_shortcode_demo_2"
); /*------------------------------------*\
	Custom Post Types
\*------------------------------------*/ // Place [html5_shortcode_demo_2] in Pages, Posts now. // Shortcodes above would be nested like this - // [html5_shortcode_demo] [html5_shortcode_demo_2] Here's the page title! [/html5_shortcode_demo_2] [/html5_shortcode_demo] // Create 1 Custom Post type for a Demo, called HTML5-Blank
function create_post_type_html5()
{
  ...
  $name = 'Place category';
  register_taxonomy('type', array('locations'), array(
    'hierarchical' => true,
    'labels' => array(
      'name' => $name,
      'singular_name' => $name,
      'search_items' =>  sprintf("Search %s", $name),
      'all_items' => sprintf("Search %ser", $name),
      'menu_name' => $name,
    ),
    'show_ui' => true,
